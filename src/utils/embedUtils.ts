import { isLocalMdFile, resolveDocPath, readMdFileContent, getFileDisplayName, normalizePath, getFileName } from './linkUtils';
...
export async function renderEmbedContent(
  mdPath: string,
...
export function setupEmbedLazyLoading(container: HTMLElement): void {
  const embedContainers = container.querySelectorAll('.embed-container:not(.embed-container--loading)');
