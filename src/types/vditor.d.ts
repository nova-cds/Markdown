 ...
 * 必须通过 `declare global` 扩展。
 *
 * 而 Vditor 类本身在 dist/index.d.ts 模块内通过 `declare class Vditor extends VditorMethod` 导出，
 * 是 class 而非 interface。TypeScript 的 `declare module 'viditor' { interface Vditor {...} }`
 * 不能合并到 class 的实例类型，因此这里通过导出交叉类型 ExtendedVditor 来表达扩展，
 * 在使用 vditorRef 的位置显式标注为 ExtendedVditor。
 */
 ...
 */
export interface VditorExtensions {
...
/**
 * 扩展后的 Vditor 类型
 * 在 vditorRef、参数类型等位置使用此类型，即可访问自定义挂载的 _xxx 属性
 */
export type ExtendedVditor = Vditor & VditorExtensions;
