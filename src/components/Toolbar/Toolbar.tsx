    ...
    <>
      <div className="h-11 bg-[var(--toolbar-bg)] border-b border-[var(--editor-border)] flex items-center px-3 gap-1 select-none">
        ...
        <div className="flex items-center gap-0.5">
          <ToolbarButton icon={FilePlus} title="新建文档" onClick={handleNewFile} />
          <ToolbarButton icon={FileText} title="打开文件" onClick={handleOpenFile} />
          <ToolbarButton icon={FolderOpen} title="打开文件夹" onClick={handleOpenFolder} />
        </div>
        ...
        <div className="ml-auto flex items-center gap-0.5">
          {/* 快捷键提示 */}
          <ToolbarButton icon={Command} title="快捷键" onClick={() => setShowShortcuts(true)} />
...
const ToolbarButton: React.FC<ToolbarButtonProps> = ({
  ...
  return (
    ...
    >
      ...
      >
        {title}
        {shortcut && <span className="ml-1.5 opacity-70 text-[10px]">{shortcut}</span>}
      </div>
    </button>
  );
};
